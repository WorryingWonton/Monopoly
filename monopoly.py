...
6 November, 2018
Version 2 Goals:
    -Version 2 will allow an Active Player to perform multiple actions per turn
        -Players can perform as many actions as they'd like, so long as their state permits those actions to be performed
            +The Option List will need to regenerate each time the Player chooses an Option
        -Options presented to the Active Player are generated by Tile.tile_actions()
            -The problem with regenerating the Option List is Tile.tile_actions() almost always has side effects.
                -There are automated actions to be performed on the Active Player when they land on a Tile. 
                 Calling Tile.tile_actions() each time an Option is selected would cause said automatic actions to 
                 affect the Active Player multiple times - Effectively breaking the game.
                -The solution is to split Tile.tile_actions() into two methods:
                    -Tile.perform_auto_actions():  Reads game state, modifies game state, returns None.
                    -Tile.list_options():  Reads game state, returns a list of Option objects.
